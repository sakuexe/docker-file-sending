...
while [[ $# -gt 0 ]]; do
  ...
  case $key in
    -p|--port)
      # if the user wants to use a custom port
      port="$2"
      shift 2  # Consume both the option and its argument
      ;;
    -n|--network)
      # if the user wants to use a custom network name
      network="$2"
      shift 2
      ;;
    -m|--mount)
      # if the user wants to mount a custom path for the volume
      mount_path="$2"
      shift 2
      ;;
    -x|--execute)
      # if the user wants to just execute the script without
      # building the image, creating the network or volume
      execute=true
      shift 1
      ;;
    -d|--debug)
...
if [ "$execute" = false ]; then
  ...
  if [ -n "$mount_path" ]; then
    ...
    docker volume create -o type=none -o device=$mount_path -o o=bind clientvol
...
# run the container
docker run -d --rm --name client --network $network -e PORT=$port sakuexe/client-image
echo -e "Client is running on port ${GREEN}$port${RESET} and network ${GREEN}$network${RESET}"
